...
>
> **当前状态**:M1–M5 ✅(2026-04-30);M6–M9 ✅(2026-05-01)。**整个 feature 已完成 + 归档**,本文件位于 `archive/`,作为只读历史。
...
- `web/dist/` 由 `npm run build` 构建,产物 import 路径不脏(沿用 v0.5.0)
...
| **M7** 多源剪辑操作 + 跨轨拖动 | ✅ 完成 | 2026-05-01 | `0d75dff` | split / delete / trim / 范围选区 / 重排 / 撤销重做在多轨模型上跑通(完全靠 M4 共享 composable + 多轨 store 实现);**跨轨拖动**(同类型轨之间;视频→音频禁止);splitScope 扩展到 `all/video/audio/track:<id>`;轨道删除二次确认;`multitrack/domain/timeline_test.go` 覆盖多轨场景;锁 `Ctrl+L` 折叠素材库(`+视频轨` / `+音频轨` 工具条按钮 M6 已落顶栏,M7 不重复)。后端新增 `multitrack/domain/tracks.go`(`RemoveVideoTrack` / `RemoveAudioTrack` / `MoveClipAcrossTracks` + `ErrCrossKindMove`)+ 测试覆盖。前端新增 `useMultitrackOps` + `MultitrackToolbar.vue`(顶栏抽出);`useTimelineDrag` 加 `findTargetTrack/onCrossTrack` 跨轨钩子;`useTimelinePlayback` 加 `extraBindings` 让多轨注入 `Ctrl+L`;multitrack store 加 selection / splitScope / rangeSelection / libraryCollapsed / undo;`go test ./...` + `CGO_ENABLED=0 go test ./...` + `vue-tsc` + `npm run build` 全绿;用户手测清单 11 项全过(单视频 Tab 零回归 + 跨轨 V→A 拒绝 + 撤销恢复 + Ctrl+L 折叠 + 自动保存往返) |
| **M8** 导出 v1:overlay + amix | ✅ 完成 | 2026-05-01 | `6d739a5` | `multitrack/domain/{filter,export}.go` 按 [program.md §5](../../tabs/multitrack/program.md) 装配 filter graph:多源 `BuildMultitrackVideoTrackFilter` / `BuildMultitrackAudioTrackFilter` 把每条轨 trim/concat 出 `[Vk]` / `[Aj]`;N≥2 视频轨链式 `overlay=0:0` → `[V]`(轨号小=底);N≥2 音频轨 `amix=inputs=N:duration=longest:dropout_transition=0` → `[A_pre]`(可选 `volume=g` → `[A]`);单轨退化时直出 `[V]` / `[A]` 不发 overlay/amix;每段视频跑 `scale=W:H:force_original_aspect_ratio=decrease,pad,setsar=1,format=yuv420p` 统一 canvas(max W × max H × max FR);短轨 trailing `color`/`anullsrc` pad 到 programDur;视频轨开头留空逐条 error 带 `videoTracks[i]`;只引用了的 source 才进 `-i`。后端 `multitrack/api/{handlers_export.go, dto.go, routes.go}` 镜像 editor 的 dryRun + 409 覆盖 + JobRunner 单实例。`export_test.go` 16 个用例覆盖 §5.3 全矩阵 + 边界。前端 `api/multitrack.ts` 加 `MultitrackExportBody/Response + exportPreview/startExport/cancelExport`;multitrack store 加 `exportLocked` + 生命周期重置;`MultitrackView.vue` 接入 `ExportDialog` + `ExportSidebar` + `useJobPanel`,顶栏导出按钮 + 前端预检(全空 / 视频轨开头留空)+ `modals.showCommand` 命令预览 + sidebar 阻断编辑(timeline mousedown / drop / 顶栏按钮 / 删除轨按钮 / 快捷键全锁)+ `watch(job.running)` 异常退出兜底解锁。`go test ./...` + `CGO_ENABLED=0 go test ./...` + `vue-tsc --noEmit` + `vite build` 全绿;用户手测清单 14 项全过(单视频单 clip 等价、双轨叠加、amix、跨源、多分辨率、视频轨开头报错、仅视频/仅音频、命令预览、覆盖确认、取消、编辑阻断、自动保存、单视频 Tab 零回归、JobRunner 互斥) |
| **M9** 收尾 + 归档 | ✅ 完成 | 2026-05-01 | _本次提交_ | 本文件 `git mv` 至 `archive/feature-v0.5.0_multitrack-editor.md`;主索引 [../../milestones.md](../../milestones.md) 中"进行中"挪到"已归档";`docs/todo/feature-v0.5.0_multitrack-editor.md` 删除;[../../README.md](../../README.md) `tabs/` 表多轨行从 🚧 → ✅ + 目录树补 `tabs/multitrack/`;`../../../README.md` 功能段加多轨剪辑器一行;`../../../CLAUDE.md` 双入口拓扑共享层 + 关键目录补 `multitrack/` + `editor/common/` + 测试段补对应 `_test.go`;[../../roadmap.md](../../roadmap.md) §1 当前版本 → v0.5.0、进行中清空,§3 加"多轨剪辑进阶"非目标行(PiP/位置/缩放/不透明度/关键帧/转场/调色,留 v0.7.x),§4 加 0.5.0 行;[../../tabs/editor/product.md §1.2](../../tabs/editor/product.md) 多素材 / 多轨叠加两条划线 + 注"v0.5.0 由多轨剪辑 Tab 提供";版本号 bump 至 **0.5.0**(`web/package.json` / `internal/version/version.go` / `cmd/desktop/wails.json`);三绿(`go test ./...` + `CGO_ENABLED=0 go test ./...` + `vue-tsc --noEmit && vite build`)+ 用户手测单视频 Tab 零回归 + 多轨基本流程一遍 |
