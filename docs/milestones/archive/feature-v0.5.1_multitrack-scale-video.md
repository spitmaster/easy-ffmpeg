...
>
> **当前状态**:M1 / M2 / M3 / M4 / M5 / M6 ✅(2026-05-04);v0.5.1 已发布。
...
| **M4** 前端工程画布 UI | ✅ 完成 | 2026-05-04 | _本次提交_ | `web/src/components/multitrack/CanvasSettingsDialog.vue`(W/H/FR 三数字输入 + 预设按钮 1080p / 4K UHD / 9:16 / 使用源最大值 + 输入校验 W/H ≥ 16 与 FR ∈ (0, 240] + OOB clip 二次确认);`MultitrackView.vue` 顶栏在"关闭工程"右侧加"画布: WxH @ FRfps ▾"按钮(`v-if="hasProject"`,`disabled="exportLocked"`);`MultitrackPreview.vue` 容器改造为画布盒(外层 shell + 内层 canvas-box,`ResizeObserver` 监听 shell 尺寸 → 等比 fit 缩放 + 8px padding;`<video>` 改 `object-fill`;音频元素移到画布盒内);新工程后端默认 1920×1080@30(`NewProject` 已设);旧工程后端 Migrate 透传(M3 已落);`stores/multitrack.ts` 加 `setCanvas(canvas)` action + `MultitrackSnapshot.canvas` 字段(`snapshotTracks` → `snapshotProject`,undo/redo 同步还原 canvas);新增 `web/src/utils/multitrack-canvas.ts`(`deriveCanvasFromSources` 镜像后端 + `clipsOutOfBounds` 计算 + `isValidCanvas` 校验);`vue-tsc --noEmit` + `vite build` + `go test ./...` + `CGO_ENABLED=0 go test ./...` 全绿 |
| **M5** 前端 clip 变换 UI | ✅ 完成 | 2026-05-04 | _本次提交_ | `web/src/components/multitrack/TransformOverlay.vue`(8 手柄 + 中心拖动 + Shift 锁纵横比 + Alt 中心对称缩放 + canvas-px ↔ DOM-px 换算 + Pointer Events + setPointerCapture);`MultitrackInspector.vue`(画布段:只读摘要 + "修改画布…" 按钮转发 → CanvasSettingsDialog;选中 clip 段:X/Y/W/H 四数字输入 + 草稿模式 + 失焦/Enter 提交 + Esc 还原 + "重置为全画布" 按钮 + 出界 ⚠ 提示;折叠成 36px 窄条);`stores/multitrack.ts` 加 `previewClipTransform`(就地变更,不入栈、不 dirty)+ `commitClipTransform`(applyProjectPatch + undoStack.push)+ 派生 `selectedVideoClip` computed(过滤掉音频选区);`MultitrackPreview.vue` 在画布盒内渲染 `TransformOverlay`(`v-if="overlayVisible && store.selectedVideoClip"`,导出锁时隐藏);`MultitrackView.vue` 挂 Inspector(`v-show="!exportFlow.sidebarOpen.value"` 与 ExportSidebar 互斥);键盘 capture-phase 监听 `onTransformKeyCapture`(选中视频 clip 时拦截 ←/→/↑/↓ 1px,Shift+ 10px;Ctrl+0 重置;`stopImmediatePropagation` 让 useTimelinePlayback 的 seek 不触发);Inspector / Overlay 在 export 锁定时全部禁用;`vue-tsc --noEmit` + `vite build` + `go test ./...` + `CGO_ENABLED=0 go test ./...` + `go build ./...` 全绿 |
| **M6** 收尾 + 归档 | ✅ 完成 | 2026-05-04 | _本次提交_ | `web/dist/` 重建(`vue-tsc --noEmit && vite build` 干净);版本号 bump 至 **v0.5.1**(`web/package.json`:0.5.0→0.5.1;`cmd/desktop/wails.json` productVersion:0.5.0→0.5.1;`internal/version/version.go` 已是 0.5.1);本文件 `git mv` 至 `archive/feature-v0.5.1_multitrack-scale-video.md`;主索引 [../milestones.md](../../milestones.md) 中"进行中"挪到"已归档";`docs/todo/feature-v0.5.1_multitrack-scale-video.md` 删除;[../roadmap.md](../../roadmap.md) §1 当前版本 → v0.5.1 + §4 已发布版本加 v0.5.1 主题行 + §3 多轨进阶非目标行已经在 v0.5.1 启动时改过("PiP / position / scale v0.5.1 提前");[../tabs/multitrack/product.md](../../tabs/multitrack/product.md) 顶段实现状态行 "v0.5.1 进行中" → "v0.5.1 已发布";三绿(`go test ./...` + `CGO_ENABLED=0 go test ./...` + `vue-tsc --noEmit && vite build`) |
