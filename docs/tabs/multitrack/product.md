...
>
> **实现状态**:📋 PRD 完成(M1 交付物);进入 M2 技术设计 → M3/M4 共享层抽取 → M5+ 多轨落地。
>
...
- 文字 / 贴纸 / 水印
- 视频 PiP 的 `position` / `scale` / `opacity` 字段(占位:轨号代表 z-order,所有视频轨默认全屏覆盖;PiP 留给 v2)
...
│ + 导入   │              │                                │                   │
│          │              │         预览窗口                │                   │
│ ▶ 🎬 vid1│              │   多轨叠加显示:vid1 在底,    │ ← 预览区(中央)
│   1080p  │              │   vid2 当前轨号更高 → 顶层    │   (单 / 多视频元素)
│   12.3 s │              │                                │
│          │              └────────────────────────────────┘                   │
│ ▶ 🎬 vid2│                                                                  │
...
│   stereo │──────┼──────┼─────────────────────────────────────────────────────│
│   30.0 s │🎬 V2 │ [+轨]│            ┌──vid2──┐                              │ ← 视频轨 V2(z-order 高)
│          │🎬 V1 │ [+轨]│ ┌─vid1──┐ ┌──vid1──┐  ┌──vid2──┐                  │ ← 视频轨 V1(默认)
...
- **轨道列**:从两条固定(V/A)→ 任意 N 条;垂直方向滚动条;轨道顺序**视频在上、音频在下**(分两组),每组内**轨号小的在下面**(z-order 低)
...
**关键技术决策**(M2 锁定;v1 默认走 6.1):

### 6.1 v1 默认方案:**单 `<video>` 切源 + 独立 `<audio>` 通道**

- 保留**一个**主 `<video>` 元素,在播放头进入不同 source 区间时切 `src` + seek
- 多视频轨叠加场景:**当前节目时间下只播放轨号最高的非空 video clip 所在的 source**(下层被遮挡 → 预览端不渲染下层 = 黑屏 + 显示遮挡警告;**导出端始终精确**)
- 多音频轨:**多个独立 `<audio>` 元素**(每条音频轨一个),用 WebAudio `GainNode` 接到混合输出;每条轨独立 `currentTime` 同步
- gap clock:沿用单视频(预览端遇到所有视频轨都空 → 黑屏 + rAF 推进 playhead)

**代价**:多视频轨叠加在预览端不真合成,只看到顶层。**接受这个不一致性**,导出始终精确(`overlay` filter)。UI 给一个明确提示:`预览仅显示顶层视频轨;实际叠加效果以导出为准`。

### 6.2 v2(后续):多 `<video>` + CSS 层叠近似

- 视频轨 → 多个 `<video>` DOM 元素层叠(`position: absolute; z-index = 轨号`)
- `requestVideoFrameCallback` 同步多个视频元素的播放头
- 仍非帧精确,但**预览也能看到叠加效果**

### 6.3 v3(远期):Canvas + WebCodecs
...
| 多源 / 多轨 / 跨轨拖动 | ✗ | ✓ **核心新增** |
| 素材库 | ✗ | ✓ **核心新增** |
| 视频叠加 / overlay | ✗ | ✓ 导出端真合成,预览端 v1 仅显示顶层 |
| PiP / 位置 / 缩放 / 不透明度 | ✗ | ⏳ v2 |
| 转场 / 调色 / 关键帧 | ✗ | ✗ |
...
- v2:视频 PiP(`position` / `scale` / `opacity` / 旋转)+ 多 `<video>` 同步预览
